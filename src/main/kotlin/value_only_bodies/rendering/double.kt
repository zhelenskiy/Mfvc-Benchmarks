...
fun Point2d?.toVector(): Vector2d =
    Vector2d(this!!.x, y)

fun Point2d?.toLocation(): LocationD =
    LocationD(this!!.x.toInt(), y.toInt())

operator fun Point2d?.minus(vector: Vector2d?): Point2d =
    Point2d(this!!.x - vector!!.x, y - vector.y)

operator fun Point2d?.plus(vector: Vector2d?): Point2d =
    Point2d(this!!.x + vector!!.x, y + vector.y)

@JvmInline
value class Vector2d(val x: Double, val y: Double)

fun Vector2d?.yProjection(): Vector2d =
    Vector2d(0.0, this!!.y)

fun Vector2d?.xProjection(): Vector2d =
    Vector2d(this!!.x, 0.0)

fun Vector2d?.abs(): Vector2d =
    Vector2d(abs(this!!.x), abs(y))

operator fun Vector2d?.minus(vector: Vector2d?): Vector2d =
    Vector2d(this!!.x - vector!!.x, y - vector.y)

operator fun Vector2d?.plus(vector: Vector2d?): Vector2d =
    Vector2d(this!!.x + vector!!.x, y + vector.y)

operator fun Vector2d?.times(factor: Double): Vector2d =
    Vector2d(this!!.x * factor, y * factor)

fun Vector2d?.rotate(angle: Double): Vector2d =
    Vector2d(this!!.x * cos(angle) - y * sin(angle), x * sin(angle) + y * cos(angle))

@JvmInline
value class LocationD(val x: Int, val y: Int)

fun LocationD?.step(vector: Vector2d?): LocationD =
    LocationD((toVector() + vector).x.toInt(), (toVector() + vector).y.toInt())

fun LocationD?.toVector(): Vector2d =
    Vector2d(this!!.x.toDouble(), y.toDouble())
...
class MyPanelD : JPanel(), KeyListener, MouseListener {
    ...
    var posImpl: Point2d? = Point2d(22.0, 12.0) // start position

    var dirImpl: Vector2d? = Vector2d(-1.0, 0.0) // direction vector

    var planeImpl: Vector2d? = Vector2d(0.0, 0.66) //the 2d raycaster version of camera plane

    inline var pos: Point2d
        get() = posImpl!!
        set(value) { posImpl = value }

    inline var dir: Vector2d
        get() = dirImpl!!
        set(value) { dirImpl = value }

    inline var plane: Vector2d
        get() = planeImpl!!
