package value_only_bodies.rendering
...
fun Point2f?.toVector(): Vector2f =
    Vector2f(this!!.x, y)

fun Point2f?.toLocation(): LocationF =
    LocationF(this!!.x.toInt(), y.toInt())

operator fun Point2f?.minus(vector: Vector2f?): Point2f =
    Point2f(this!!.x - vector!!.x, y - vector.y)

operator fun Point2f?.plus(vector: Vector2f?): Point2f =
    Point2f(this!!.x + vector!!.x, y + vector.y)

@JvmInline
value class Vector2f(val x: Float, val y: Float)

fun Vector2f?.yProjection(): Vector2f =
    Vector2f(0.0f, this!!.y)

fun Vector2f?.xProjection(): Vector2f =
    Vector2f(this!!.x, 0.0f)

fun Vector2f?.abs(): Vector2f =
    Vector2f(abs(this!!.x), abs(y))

operator fun Vector2f?.minus(vector: Vector2f?): Vector2f =
    Vector2f(this!!.x - vector!!.x, y - vector.y)

operator fun Vector2f?.plus(vector: Vector2f?): Vector2f =
    Vector2f(this!!.x + vector!!.x, y + vector.y)

operator fun Vector2f?.times(factor: Float): Vector2f =
    Vector2f(this!!.x * factor, y * factor)

fun Vector2f?.rotate(angle: Float): Vector2f =
    Vector2f(this!!.x * cos(angle) - y * sin(angle), x * sin(angle) + y * cos(angle))

@JvmInline
value class LocationF(val x: Int, val y: Int)

fun LocationF?.step(vector: Vector2f?): LocationF =
    LocationF((toVector() + vector).x.toInt(), (toVector() + vector).y.toInt())

fun LocationF?.toVector(): Vector2f =
    Vector2f(this!!.x.toFloat(), y.toFloat())
...
class MyPanelF : JPanel(), KeyListener, MouseListener {
    ...
    var posImpl: Point2f? = Point2f(22.0f, 12.0f) // start position

    var dirImpl: Vector2f? = Vector2f(-1.0f, 0.0f) // direction vector

    var planeImpl: Vector2f? = Vector2f(0.0f, 0.66f) //the 2d raycaster version of camera plane

    inline var pos: Point2f
        get() = posImpl!!
        set(value) { posImpl = value }

    inline var dir: Vector2f
        get() = dirImpl!!
        set(value) { dirImpl = value }

    inline var plane: Vector2f
        get() = planeImpl!!
        set(value) { planeImpl = value }
